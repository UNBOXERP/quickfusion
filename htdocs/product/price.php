	...
	if (!empty($socid)) {
		...
		for ($i = 1; $i <= $conf->global->PRODUIT_MULTIPRICES_LIMIT; $i++) {
			...
			if (preg_match('/editlabelsellingprice/', $action)) {
                 ...
                 * */
                if($i==1){
                    print $langs->trans("Selling_price_1");
                }elseif($i==2){
                    print $langs->trans("Selling_price_2");
                }elseif($i==3){
                    print $langs->trans("Selling_price_3");
                }elseif($i==4){
                    print $langs->trans("Selling_price_4");
                }elseif($i==5){
                    print $langs->trans("Selling_price_5");
                }elseif($i==6){
                    print $langs->trans("Selling_price_6");
                }elseif($i==7){
                    print $langs->trans("Selling_price_7");
                }else{
					//print $langs->trans("SellingPrice").' '.$i;  //original
					if (!empty($conf->global->$keyforlabel)) {
						$test=$langs->trans($conf->global->$keyforlabel);
						if($conf->global->$keyforlabel==''){

						}

						$test=print ' - '.$langs->trans($conf->global->$keyforlabel);
						$asd=0;
					}
				}
...
if ($action == 'edit_price' && $object->getRights()->creer) {
	...
	if (empty($conf->global->PRODUIT_MULTIPRICES) && empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES)) {
		...
		for ($i = 1; $i <= $conf->global->PRODUIT_MULTIPRICES_LIMIT; $i++) {
			...
			//alan cambio edit
			$text = $langs->trans('Selling_price_'.$i.'');
			print $form->textwithpicto($text, $langs->trans("PrecisionUnitIsLimitedToXDecimals", $conf->global->MAIN_MAX_DECIMALS_UNIT), 1, 1);
			print '</td>';
