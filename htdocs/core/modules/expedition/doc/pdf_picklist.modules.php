 ...
 */
class pdf_picklist extends ModelePdfExpedition
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$sql4 = " SELECT p.ref FROM ".MAIN_DB_PREFIX."commandedet AS cd ";
                ...
                $sql4.= " LEFT JOIN ".MAIN_DB_PREFIX."expeditiondet AS exde ON exde.fk_origin_line = cd.rowid ";
                $sql4.= " WHERE  exde.fk_expedition =  " . $object->id;
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					pdf_writelinedesc($pdf,  '', $i, $outputlangs, $this->posxpicture - $curX, 3, $curX, $curY, $hideref =1, $hidedesc = 1);
					$pdf->MultiCell($this->posxtva - $curX-75, 3, $note[$i][0], $curX, $curY, 0);
					...
					if ($pageposafter > $pageposbefore) {	// There is a pagebreak
						...
						pdf_writelinedesc($pdf, '', $i, $outputlangs, $this->posxpicture - $curX, 3, $curX, $curY, $hideref, $hidedesc);
						$pdf->MultiCell($this->posxtva - $curX-75, 3, $note[$i][0], $curX, $curY, 0);
					...
					//	$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 3, $object->lines[$i]->qty_shipped, '', 'C');
					}

					if (empty($conf->global->SHIPPING_PDF_HIDE_QTYTOSHIP)) {
						$pdf->SetXY($this->posxqtytoship, $curY);
						$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 3, $object->lines[$i]->qty_asked, '', 'C');
					}
	...
	/*	if (empty($conf->global->SHIPPING_PDF_HIDE_WEIGHT_AND_VOLUME)) {
		...
		/*if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED)) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 2, $outputlangs->transnoentities("QtyToShip"), '', 'C');
			}
		}*/
		...
		if (empty($conf->global->SHIPPING_PDF_HIDE_QTYTOSHIP)) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 2, $outputlangs->transnoentities("QtyOrdered"), '', 'C');
			}
		}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->MultiCell($w, 4, 'Picking List', '', 'R');
