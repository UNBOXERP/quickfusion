 ...
 */
class pdf_ein3 extends ModelePDFCommandes
{
	/**
		...
		if ($conf->commande->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				// Loop on each lines
                     ...
                     $sql4 = " SELECT cex.notes,  CONCAT_WS(p.ref,' ',p.label,' ',p.description), cd. price, cd.qty, cd.total_ht, cd.description,  CASE(pex.manuty) WHEN 1 THEN 'Laser Etched' WHEN 2 THEN 'Digital Print' END AS manuty  FROM ".MAIN_DB_PREFIX."commandedet AS cd ";
                ...
                $sql4.= " WHERE  cd.fk_commande =  " . $object->id . " ORDER BY pex.manuty DESC ";
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$pdf->MultiCell($this->posxtva - $curX-75, 3, $note[$i][1].''. $note[$i][5] .' - '. $note[$i][6], $curX, $curY, 0);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						$pdf->MultiCell($this->posxtva - $curX-75, 4, $note[$i][1].''. $note[$i][5] .' - '. $note[$i][6], $curX, $curY, 0);
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
					...
					$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 3, "rey", 0, 'R', 0);
