 ...
 */
class pdf_conford extends ModelePDFCommandes
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity])
		{
			...
			if ($object->specimen)
			{
                ...
                $file = $dir."/CO_".$objectref."_". $dat7 . "_".$dat4."_" .$dat2.".pdf"; //alan aimee
			}
			...
			if (file_exists($dir))
			{
				...
				$sql4 = " SELECT cex.notes,  CONCAT_WS(' ',p.label,p.description), cd. price, cd.qty, cd.total_ht, cd.description as cdp, p.description,  row_number() over(order by p.description DESC) AS rowno, p.ref, CASE (cp.fk_categorie) 
WHEN '2'  THEN '0'
WHEN '38' THEN '0'
WHEN '1' THEN '01'
WHEN '3' THEN '03'
...
WHEN '8' THEN '08'
WHEN '9' THEN '09'
ELSE cp.fk_categorie 
END AS orde  FROM ".MAIN_DB_PREFIX."commandedet AS cd ";
                ...
                $sql4.= " WHERE  cd.fk_commande =  " . $object->id . " ORDER BY orde ";
    ...
    pdf_writelinedesc($pdf, '', $i, $outputlangs, $this->posxtva - $curX-55, 4, $curX, $curY-5, $hideref, $hidedesc);
				...
				//	$pdf->SetTextColor(204, 0, 0);
					...
					// Quantity
                       ...
                       // $pdf->SetTextColor(204, 0, 0);
					$pdf->SetXY($this->posxqty +18, $curY);
					$pdf->MultiCell($this->posxunit - $this->posxqty - 0.8, 4, number_format($qty), 0, 'C'); // Enough for 6 chars
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		$pdf->MultiCell($w, 3, "Confirmation Order", '', 'R');
