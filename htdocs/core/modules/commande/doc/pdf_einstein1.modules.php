 ...
 */
class pdf_einstein1 extends ModelePDFCommandes
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->commande->multidir_output[$conf->entity])
		{
			...
			if ($object->specimen)
			{
				...
				//$file = $dir."/SO_".$objectref.".pdf";
				 $file = $dir."/SO_".$objectref."_". $dat7 . "_".$dat4."_" .$dat2.".pdf";
			}
			...
			if (file_exists($dir))
			{
                ...
                $sql4.= " WHERE  cd.fk_commande =  " . $object->id . " ORDER BY p.description DESC ";
...
$pdf->MultiCell($this->posxtva - $curX-75, 4,$note[$i][7].'.- '. $note[$i][8], $curX, $curY, 0);
					...
					$pdf->MultiCell($this->posxqty - $this->posxup - 0.8, 3, $note[$i][2], 0, 'R', 0);
					...
					$pdf->MultiCell($this->page_largeur - $this->marge_droite - $this->postotalht, 3,number_format($note[$i][4],2,'.',','), 0, 'R', 0);
