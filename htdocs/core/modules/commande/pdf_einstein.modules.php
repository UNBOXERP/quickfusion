		...
		if ($conf->commande->multidir_output[$conf->entity])
		{
			...
			if (file_exists($dir))
			{
				...
				// Loop on each lines
                     ...
                     $sql4 = " SELECT cex.notes  FROM ".MAIN_DB_PREFIX."commandedet AS cd ";
                $sql4.= " LEFT JOIN ".MAIN_DB_PREFIX."commandedet_extrafields AS cex ON cd.rowid = cex.fk_object ";
                $sql4.= " WHERE  cd.fk_commande =  " . $object->id;
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxtva - $curX-75, 3, $curX, $curY, $hideref, $hidedesc);
					...
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						...
						pdf_writelinedesc($pdf, $object, $i, $outputlangs, $this->posxtva - $curX-75, 4, $curX, $curY, $hideref, $hidedesc);
						$pageposafter = $pdf->getPage();
						$posyafter = $pdf->GetY();
