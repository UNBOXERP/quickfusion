	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				//if (! empty($conf->global->FACTURE_LOCAL_TAX2_OPTION) && $conf->global->FACTURE_LOCAL_TAX2_OPTION=='localtax2on')
				//{
				$sql=  "SELECT  DATE_FORMAT(f.datef, '%Y-%m-%d') AS ship_date , CONCAT(sp.firstname,' ',sp.lastname) AS contact, f.ref AS fa,  soc.code_client as brooker, fex.shpping AS ship ";
                    $sql.= " FROM ".MAIN_DB_PREFIX."facture AS f ";
                    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."societe AS soc ON f.fk_soc = soc.rowid ";
                    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."facture_extrafields AS fex ON f.rowid = fex.fk_object ";
                    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."element_contact AS ec ON ec.element_id = f.rowid AND ec.fk_c_type_contact = 60 ";
                    ...
                    foreach($resql as $value){
						...
						$totalvat .= vatrate(abs($tvakey), 1).$tvacompl;
						$pdf->MultiCell($col2x - $col1x, $tab2_hl, "Shipping ", 0, 'L', 1);

						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
						$pdf->MultiCell($largcol2, $tab2_hl, price($dat2, 0, $outputlangs), 0, 'R', 1);
