...
<script>
    if (window.location.href.indexOf("commande/card.php") > -1) {

    //alert("solo commande");
    //document.getElementById("idprod").onchange = function() {myFunction()};

    // idprod
        var orden = getParameterByName('id');
        //alert("1solo commande: "+orden);
    $("#idprod").change(function(){
        //alert($("#idorden").val());
        let token ='<?php echo currentToken(); ?>';

        let id=this.value;

        $.ajax({
            url: "../../custom/multiprecios/operaciones.php",
            type: "POST",
            data: {id: id, orden:orden},
            success: function (data1) {
                var valor = JSON.parse(data1);
                defaultprice = valor.valor;
                // alert(defaultprice);
               // price1 = valor.valor1;

                        $("input[name='price_ht']:first").val(defaultprice);
