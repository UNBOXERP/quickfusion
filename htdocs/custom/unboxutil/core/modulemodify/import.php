...
if ($step == 3 && $datatoimport) {
	if (GETPOST('sendit') && !empty($conf->global->MAIN_UPLOAD_DOC)) {
		...
		$fullpath = $conf->import->dir_temp."/".$nowyearmonth.'-'.$user->login.'-'.$_FILES['userfile']['name'];
...
if ($step == 3 && $datatoimport) {
	...
	if (count($filearray) > 0) {
		...
		foreach ($filearray as $key => $val) {
            ...
            $valores=posix_getpwuid(fileowner($file));
            print '<td width="16">'.$porciones['0'].'</td>';
            print '<td width="16">'.$porciones['1'].'</td>';
