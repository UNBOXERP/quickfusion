...
$sql.= ", d.payment_cond";
$sql.= ", d.priority";
$sql.= ", p.rowid as socid, p.ref_client, p.".$refDoli9or10." as name";
$sql.= ", 1 AS invoice";
$sql.= " FROM ".MAIN_DB_PREFIX."discount AS d";
$sql.= " , ".MAIN_DB_PREFIX."facture AS p";

$sql.= " WHERE ((d.type_source = ".Discounts::SOURCE_INVOICE." AND p.rowid=d.fk_source) OR (p.rowid=d.fk_target AND d.type_target = ".Discounts::SOURCE_INVOICE."))";
$sql.= " AND d.entity = ".$conf->entity;
if ($snom)     $sql.= " AND p.".$refDoli9or10." LIKE '%".$db->escape($snom)."%'";
...
print_liste_field_titre($langs->trans("Invoice"), $_SERVER["PHP_SELF"], "name",$param,"","",$sortfield,$sortorder);
...
while ($i < min($num,$limit)) {
    ...
    // Company
    if ($objp->invoice == 1) {
    	$soc_static->fetch($objp->socid, '');
        print '<td>';
        print $soc_static->getNomUrl(1);
        print '</td>';
    } else {
    ...
    if ($modo == 'normal') {
        if ($objp->fk_target > 0) {
            if ($objp->type_target == Discounts::SOURCE_CATEGORY) {
				...
				print '</span></td>';
            } elseif ($objp->type_target == Discounts::SOURCE_PRODUCT) {
                $cat = new Product($db);
                $cat->fetch($objp->fk_target);
