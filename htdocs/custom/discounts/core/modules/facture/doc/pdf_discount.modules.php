...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_discount.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from discount model
 */
...
/**
 *	Class to manage PDF invoice template Discount
 */
class pdf_discount extends pdf_crabe
{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					if($applypromo){
						if(isset($promo_desc)){
							...
							$pdf->writeHTMLCell($this->posxtva-$curX, 3, $curX, $curY, $outputlangs->convToOutputCharset($outputlangs->trans("InPromo")), 0, 1, false, true, 'J',true);
						}
					}else{
					...
					if($applypromo){
						...
						$res = $dis_doc->fetch(3,$object->lines[$i]->rowid);
						if($res > 0){
	...
	public function _tableau_versements(&$pdf, $object, $posy, $outputlangs, $heightforfooter = 0)
	{
		// phpcs:enable
		global $conf;
