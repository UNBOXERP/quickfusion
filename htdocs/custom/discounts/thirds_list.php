...
$sql.= ", d.payment_cond";
$sql.= ", d.priority";
$sql.= ", p.rowid as socid, p.client, p.code_client, p.nom as name";
$sql.= ", 1 AS third";
$sql.= " FROM ".MAIN_DB_PREFIX."discount AS d";
$sql.= " , ".MAIN_DB_PREFIX."societe AS p";

$sql.= " WHERE ((d.type_source = ".Discounts::SOURCE_THIRD." AND p.rowid=d.fk_source) OR (p.rowid=d.fk_target AND d.type_target = ".Discounts::SOURCE_THIRD."))";
$sql.= " AND d.entity = ".$conf->entity;
if ($snom)     $sql.= " AND p.nom LIKE '%".$db->escape($snom)."%'";
...
$sql.= ", d.payment_cond";
$sql.= ", d.priority";
$sql.= ", p.rowid AS socid";
$sql.= ", 0 AS client";
$sql.= ", 0 AS code_client";
$sql.= ", p.label AS name";
$sql.= ", 0 AS third";
$sql.= " FROM ".MAIN_DB_PREFIX."discount AS d,";
$sql.= " ".MAIN_DB_PREFIX."categorie AS p INNER JOIN ".MAIN_DB_PREFIX."categorie_societe AS cs ON cs.fk_categorie = p.rowid";
...
print_liste_field_titre($langs->trans("Company"), $_SERVER["PHP_SELF"], "name",$param,"","",$sortfield,$sortorder);
...
while ($i < min($num,$limit)) {
    ...
    // Company
    if ($objp->third == 1) {
        $soc_static->id = $objp->socid;
        $soc_static->code_client = $objp->code_client;
        $soc_static->name = $objp->name;
        $soc_static->client = $objp->client;
        print '<td>';
        print $soc_static->getNomUrl(1, 'customer');
        print '</td>';
    } else {
		$cat_static->fetch($objp->socid);
        $cat_static->id = $objp->socid;
        $cat_static->label = $objp->name;
        $cat_static->type = Categorie::TYPE_CUSTOMER;
