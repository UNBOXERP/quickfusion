...
class reportico_report_rjson extends reportico_report
{
    ...
    function setup_report_attributes()
    {
		...
		if ( !get_request_item("printable_rjson") )
        {
            ...
            {
		     $this->jar["attributes"]["show_prepare_button"] .= $this->query->get_action_url().'?'.$forward.'execute_mode=PREPARE&reportico_session_name='.reportico_session_name()."&token=".newToken();
            }
		    if ( get_reportico_session_param("show_refresh_button") )
		     $this->jar["attributes"]["show_refresh_button"] .= $this->query->get_action_url().'?'.$forward.'refreshReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name()."&token=".newToken();
        }
        else
        {
		    $this->jar["attributes"]["show_print_button"] .= $this->query->get_action_url().'?'.$forward.'printReport=1&execute_mode=EXECUTE&reportico_session_name='.reportico_session_name()."&token=".newToken();
        }
