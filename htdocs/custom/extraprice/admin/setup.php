...
if ($action == 'setproducttvaexport') {
    ...
    $listproducttvaexport = implode(',',GETPOST('arrayproducttvaexport', 'array'));
	// save the setting
	dolibarr_set_const($db, "EXTRAPRICE_PRODUCT_TVAEXPORT", $listproducttvaexport, 'chaine', 0, '', $conf->entity);
	$mesg = "<font class='ok'>".$langs->trans("SetupSaved")."</font>";
...
$ExtraPriceFormula=!empty($conf->global->ExtraPriceFormula)?$conf->global->ExtraPriceFormula:"";
$arrayproducttvaexport=!empty($conf->global->EXTRAPRICE_PRODUCT_TVAEXPORT)?explode(",",$conf->global->EXTRAPRICE_PRODUCT_TVAEXPORT):array();
...
$arrayOfProducts = $form->select_produits_list( '', $htmlname = '', '',  20,  0, '', 1, 2, 1);
$arrayProductList=array();
foreach ($arrayOfProducts as $key => $value) {
    $arrayProductList[$key]=array(
            "id" => $value['key'],
            "label" => $value['label2'],
            "color" =>"",
            "picto" =>""
...
print $form->multiselectarray('arrayproducttvaexport', $arrayProductList, $arrayproducttvaexport, 0, 0, 'minwidth400imp maxwidth500');
...
print '<tr ><td>'.$langs->trans("DatabaseVersion").'</td><td>'.$db::LABEL." ".$db->getVersion().'</td></tr>'."\n";
print '<tr ><td>'.$langs->trans("WebServerVersion").'</td><td>'.$_SERVER["SERVER_SOFTWARE"].'</td></tr>'."\n";
print '<tr>'."\n";
