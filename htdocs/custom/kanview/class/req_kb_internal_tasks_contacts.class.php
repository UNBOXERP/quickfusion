...
/**
 * Class ReqKbInternalTasksContacts
 *
 * Put some comments here
 */
class ReqKbInternalTasksContacts extends CommonObject {
	...
	/**
	 *
	 * @var ReqKbInternalTasksContactsLine[] Lines
	 */
	public $lines = array();
	// public $prop1;
	...
	public function fetchAll($LIMIT = 0, $OFFSET = 0, $ORDERBY = '', $isNewOrderBy = true, $WHERE = '', $isNewWhere = true, $HAVING = '', $isNewHaving = true) {
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new ReqKbInternalTasksContactsLine();
	...
	public function getCodeSQL($_ORDERBY = '', $_isNewOrderBy = true, $_WHERE = '', $_isNewWhere = true, $_HAVING = '', $_isNewHaving = true) {
		...
		$sql .= " " . "" . MAIN_DB_PREFIX . "user.civility AS civility,";
		$sql .= " " . "" . MAIN_DB_PREFIX . "user.lastname AS lastname,";
		$sql .= " " . "" . MAIN_DB_PREFIX . "user.firstname AS firstname,";
		$sql .= " " . "" . MAIN_DB_PREFIX . "user.photo AS photo,";
		$sql .= " " . "" . MAIN_DB_PREFIX . "user.login AS login";

		$sql .= " FROM ";
		$sql .= "" . MAIN_DB_PREFIX . "element_contact AS t   left join " . MAIN_DB_PREFIX . "projet_task on t.element_id = " . MAIN_DB_PREFIX . "projet_task.rowid   left join " . MAIN_DB_PREFIX . "c_type_contact on t.fk_c_type_contact = " . MAIN_DB_PREFIX . "c_type_contact.rowid   left join " . MAIN_DB_PREFIX . "user on t.fk_socpeople = " . MAIN_DB_PREFIX . "user.rowid";

		// --------- WHERE
		$WHERE = "  " . MAIN_DB_PREFIX . "c_type_contact.element = 'project_task'  and  " . MAIN_DB_PREFIX . "c_type_contact.source = 'internal'";
		$WHERE = trim($WHERE);
		if (!empty($_WHERE)) {
			if ($_isNewWhere)
	...
	public function toArray() {
		$object_array	 = array();
		$fields_array	 = array('rowid', 'id', 'datecreate', 'statut', 'element_id', 'fk_c_type_contact', 'fk_socpeople', 'fk_projet', 'type_contact_element', 'type_contact_source', 'type_contact_code', 'type_contact_libelle', 'type_contact_active', 'civility', 'lastname', 'firstname', 'photo', 'login',);
...
/**
 * Class ReqKbInternalTasksContactsLine
 */
class ReqKbInternalTasksContactsLine {
