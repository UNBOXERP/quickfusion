...
class ReqKbMainProspects extends CommonObject {
	...
	public function getCodeSQL($_ORDERBY = '', $_isNewOrderBy = true, $_WHERE = '', $_isNewWhere = true, $_HAVING = '', $_isNewHaving = true) {
		...
		$sql .= "" . MAIN_DB_PREFIX . "societe as t    "
				. " left join " . MAIN_DB_PREFIX . "c_stcomm on t.fk_stcomm = " . MAIN_DB_PREFIX . "c_stcomm.id  "
		...
		// --------- WHERE
		// $WHERE = " t.fournisseur = 0   AND   (t.client = 2 OR t.client = 3)";
		$WHERE = " (t.client = 2 OR t.client = 3) "; // suppression de la condition sur les fournisseurs pour s'aligner avec la liste des prospects standard de Dolibarr
