...
/**
 * Class ReqKbMainOrders
 *
 * Put some comments here
 */
class ReqKbMainOrders extends CommonObject {
	...
	public function copyObject($objSource, $objDest) {
		...
		$objDest->date_creation			 = $this->db->jdate($objSource->date_creation);
		$objDest->date_valid				 = $this->db->jdate($objSource->date_valid);
		$objDest->date_cloture			 = $this->db->jdate($objSource->date_cloture);
		$objDest->date_commande			 = $this->db->jdate($objSource->date_commande);
		...
		$objDest->source						 = $objSource->source;
		$objDest->fk_statut					 = $objSource->fk_statut;
		$objDest->amount_ht					 = $objSource->amount_ht;
		...
		$objDest->facture						 = $objSource->facture;
		...
		$objDest->fk_warehouse			 = $objSource->fk_warehouse;
	...
	public function getCodeSQL($_ORDERBY = '', $_isNewOrderBy = true, $_WHERE = '', $_isNewWhere = true, $_HAVING = '', $_isNewHaving = true) {
		...
		$sql .= " " . "t.tms AS tms,";
		$sql .= " " . "t.date_creation AS date_creation,";
		...
		$sql .= " " . "t.source AS source,";
		$sql .= " " . "t.fk_statut AS fk_statut,";
		$sql .= " " . "t.amount_ht AS amount_ht,";
		...
		$sql .= " " . "t.fk_warehouse AS fk_warehouse,";
		...
		$sql .= " " . "COUNT(" . MAIN_DB_PREFIX . "commandedet.rowid) AS nbre_lignes,";
		...
		$sql .= " FROM ";
		$sql .= "" . MAIN_DB_PREFIX . "commande as t    left join " . MAIN_DB_PREFIX . "societe on t.fk_soc = " . MAIN_DB_PREFIX . "societe.rowid   left join " . MAIN_DB_PREFIX . "commandedet on t.rowid = " . MAIN_DB_PREFIX . "commandedet.fk_commande   left join " . MAIN_DB_PREFIX . "product on " . MAIN_DB_PREFIX . "commandedet.fk_product = " . MAIN_DB_PREFIX . "product.rowid";
		...
		$GROUPBY = "t.rowid,    id,  t.ref,    t.entity,    t.ref_ext,    t.ref_int,    t.ref_client,    t.fk_soc, "
				. "   t.fk_projet,    t.tms,    t.date_creation,    t.date_valid,    t.date_cloture,    t.date_commande,"
				. "    t.fk_user_author,    t.fk_user_modif,    t.fk_user_valid,    t.fk_user_cloture,    t.source, "
				. "   t.fk_statut,    t.amount_ht,    t.remise_percent,    t.remise_absolue,    t.remise,    " . $tva_field . ",   "
				. " t.localtax1,    t.localtax2,    t.total_ht,    t.total_ttc,    t.note_private,    t.note_public,  "
				. "  t.facture,    t.fk_currency,    t.fk_cond_reglement,    t.fk_mode_reglement,    t.date_livraison, "
				. "   t.fk_warehouse,    t.fk_availability,    t.fk_input_reason,    t.extraparams,       societe_nom,  "
				. "  societe_name_alias,   societe_logo";
		...
		// ----------- ORDER BY
		$ORDERBY = "t.date_commande DESC, t.fk_soc";
	...
	public function toArray() {
		$object_array	 = array();
		$fields_array	 = array('rowid', 'id', 'ref', 'entity', 'ref_ext', 'ref_int', 'ref_client', 'fk_soc', 'fk_projet', 'tms', 'date_creation', 'date_valid', 'date_cloture', 'date_commande', 'fk_user_author', 'fk_user_modif', 'fk_user_valid', 'fk_user_cloture', 'source', 'fk_statut', 'amount_ht', 'remise_percent', 'remise_absolue', 'remise', 'tva', 'localtax1', 'localtax2', 'total_ht', 'total_ttc', 'note_private', 'note_public', 'facture', 'fk_currency', 'fk_cond_reglement', 'fk_mode_reglement', 'date_livraison', 'fk_warehouse', 'fk_availability', 'fk_input_reason', 'extraparams', 'societe_nom', 'societe_name_alias', 'societe_logo', 'nbre_lignes', 'nbre_produits', 'nbre_services',);
