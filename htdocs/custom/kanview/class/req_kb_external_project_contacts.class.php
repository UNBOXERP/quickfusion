...
/**
 * Class ReqKbExternalProjectContacts
 *
 * Put some comments here
 */
class ReqKbExternalProjectContacts extends CommonObject {
	...
	/**
	 *
	 * @var ReqKbExternalProjectContactsLine[] Lines
	 */
	public $lines = array();
	// public $prop1;
	...
	public function fetchAll($LIMIT = 0, $OFFSET = 0, $ORDERBY = '', $isNewOrderBy = true, $WHERE = '', $isNewWhere = true, $HAVING = '', $isNewHaving = true) {
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				$line = new ReqKbExternalProjectContactsLine();
	...
	public function copyObject($objSource, $objDest) {
		...
		$objDest->projet_rowid				 = $objSource->projet_rowid;
	...
	public function getCodeSQL($_ORDERBY = '', $_isNewOrderBy = true, $_WHERE = '', $_isNewWhere = true, $_HAVING = '', $_isNewHaving = true) {
		...
		$sql .= " " . "" . MAIN_DB_PREFIX . "projet.rowid AS projet_rowid,";
		...
		$sql .= " FROM ";
		$sql .= "" . MAIN_DB_PREFIX . "element_contact AS t   LEFT JOIN " . MAIN_DB_PREFIX . "projet ON t.element_id = " . MAIN_DB_PREFIX . "projet.rowid   left join " . MAIN_DB_PREFIX . "c_type_contact on t.fk_c_type_contact = " . MAIN_DB_PREFIX . "c_type_contact.rowid   left join " . MAIN_DB_PREFIX . "socpeople on t.fk_socpeople = " . MAIN_DB_PREFIX . "socpeople.rowid";

		// --------- WHERE
		$WHERE = "  " . MAIN_DB_PREFIX . "c_type_contact.element = 'project'  and  " . MAIN_DB_PREFIX . "c_type_contact.source = 'external'";
		$WHERE = trim($WHERE);
		if (!empty($_WHERE)) {
			if ($_isNewWhere)
	...
	public function toArray() {
		$object_array	 = array();
		$fields_array	 = array('rowid', 'id', 'datecreate', 'statut', 'element_id', 'fk_c_type_contact', 'fk_socpeople', 'projet_rowid', 'type_contact_element', 'type_contact_source', 'type_contact_code', 'type_contact_libelle', 'type_contact_active', 'civility', 'lastname', 'firstname', 'photo',);
...
/**
 * Class ReqKbExternalProjectContactsLine
 */
class ReqKbExternalProjectContactsLine {
