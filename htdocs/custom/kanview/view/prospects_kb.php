...
if (empty($search_prospect_client)) {
	if (empty($conf->global->KANVIEW_PROSPECTS_ADD_PROSPECTSCLIENTS)) {
		$search_prospect_client = 2; // Prospect uniquement
	}
	else {
		$search_prospect_client = 5; // Prospect et Prospect/Client
	}
}
...
include_once dol_buildpath('/kanview/class/req_kb_main_prospects.class.php');
...
if ($num >= 0) {
	...
	while ($i < $num) {
		...
		// faut-t-il afficher les Prospects seulement, les Prospects/Clients seulement ou les deux
		// (la requête sélectionne les 2, c'est ici qu'on fait le filtre si nécessaire)
		if ($search_prospect_client != 5) {
			if (($obj->client == 3 && $search_prospect_client == 2) || ($obj->client == 2 && $search_prospect_client == 3)) {
				$i++;
				continue;
			}
		}
		...
		// Si le code du niveau du niveau du prospect possède une taduction, on l'affiche (sinon c'est le libellé défini dans le dictionnaire qui sera affcihé)
		if (!empty($obj->fk_prospectlevel) && $langs->trans($obj->fk_prospectlevel) != $obj->fk_prospectlevel)
		...
		$dataArray[$i]['tooltip_content']										 .= '<tr class="tooltip-tr"><td class="tooltip-label"><b>' . $langs->trans('ReqKbMainProspects_Fieldpays_label') . '</b></td><td>: ' . (!empty($obj->pays_label) ? $langs->trans($obj->pays_label) : '') . '</td></tr>';
		...
		// contenu
		// < 6.0.0
		$thirdpartyCard										 = ($compareVersionTo600 === -1 ? 'soc.php' : 'card.php');
		if (!empty($obj->code_client)) // pour les prospects le code/ref n'est pas obligatoire
			$dataArray[$i]['code_client_nom']	 = '<a class="object-link" href="' . DOL_URL_ROOT . '/societe/' . $thirdpartyCard . '?socid=' . $obj->rowid . '" target="_blank">' . $obj->code_client . '</a>' . $prefix . $obj->nom . ($obj->client == 3 ? ' (P/C)' : '') . $suffix;
		else
...
if ($search_prospect_client != '')
	$params .= '&amp;search_prospect_client=' . urlencode($search_prospect_client);
...
llxHeader('', $langs->trans("Kanview_KB_KanbanProspects"), $help_url, '', 0, 0, $arrayofjs, $arrayofcss, '');

$form = new Form($db);

//$head = kanview_kanban_prepare_head($params);

// dol_fiche_head($head, $tabactive, $langs->trans('Kanview_KB_KanbanProspects'), 0, 'action');
...
//
// titre du Kanban
$title = $langs->trans('Kanview_KB_KanbanProspects');
...
$SQL						 = "
					...
					FROM " . MAIN_DB_PREFIX . "c_country as t
					WHERE t.active = 1
...
$SQL									 = "
							SELECT code, label
							FROM " . MAIN_DB_PREFIX . "c_prospectlevel as t
							WHERE t.active = 1
...
//
//
// ------------- filtre --req_kb_main_prospects-- Prospect/Client
//
...
if (empty($conf->browser->phone)) {
	...
	print '<div class="legend-color" '
			. 'style="border-color: transparent transparent ' . $conf->global->KANVIEW_PROSPECTS_PL_HIGH_COLOR . ' transparent;">'
...
//			. 'border-color-left: transparent;">'
			. '</div>';
	print '</td>';
	print '<td class="legend-label">' . $langs->trans('KANVIEW_PROSPECTS_PL_HIGH_COLOR_DESC') . '</td>';
	print '</tr>';
	// -- legend 2
	print '<tr>';
	print '<td>';
	print '<div class="legend-color" '
			. 'style="border-color: transparent transparent ' . $conf->global->KANVIEW_PROSPECTS_PL_MEDIUM_COLOR . ' transparent;">'
			. '</div>';
	print '</td>';
	print '<td class="legend-label">' . $langs->trans('KANVIEW_PROSPECTS_PL_MEDIUM_COLOR') . '</td>';
	print '</tr>';
	// -- legend 3
	print '<tr>';
	print '<td>';
	print '<div class="legend-color" '
			. 'style="border-color: transparent transparent ' . $conf->global->KANVIEW_PROSPECTS_PL_LOW_COLOR . ' transparent;">'
			. '</div>';
	print '</td>';
	print '<td class="legend-label">' . $langs->trans('KANVIEW_PROSPECTS_PL_LOW_COLOR') . '</td>';
	print '</tr>';
	// -- legend 4
	print '<tr>';
	print '<td>';
	print '<div class="legend-color" '
			. 'style="border-color: transparent transparent ' . $conf->global->KANVIEW_PROSPECTS_PL_NONE_COLOR . ' transparent;">'
			. '</div>';
	print '</td>';
	print '<td class="legend-label">' . $langs->trans('KANVIEW_PROSPECTS_PL_NONE_COLOR') . '</td>';
	print '</tr>';
	// -- legend 5
	print '<tr>';
	print '<td>';
	...
	print '</td>';
	print '<td class="legend-label">' . $langs->trans(strtoupper($conf->global->KANVIEW_PROSPECTS_TAG)) . '</td>';
	print '</tr>';
...
echo '<script type="text/javascript">
		...
		var prospects_tag		= "' . trim($conf->global->KANVIEW_PROSPECTS_TAG) . '";
		var colorMapping  =  {
	        "' . trim($conf->global->KANVIEW_PROSPECTS_PL_HIGH_COLOR) . '": "PL_HIGH",
	        "' . trim($conf->global->KANVIEW_PROSPECTS_PL_LOW_COLOR) . '": "PL_LOW",
	        "' . trim($conf->global->KANVIEW_PROSPECTS_PL_MEDIUM_COLOR) . '": "PL_MEDIUM",
	        "' . trim($conf->global->KANVIEW_PROSPECTS_PL_NONE_COLOR) . '": "PL_NONE",
	      };
...
function kanview_kanban_prepare_head($params) {
	...
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'kanview_prospects_kanban');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'kanview_prospects_kanban', 'remove');
...
function getDepartementsOptions($idpays = 0, $search_fk_departement = 0) {
	...
	$SQL = "SELECT t.rowid, t.nom, t.fk_region, " . MAIN_DB_PREFIX . "c_regions.fk_pays
					FROM " . MAIN_DB_PREFIX . "c_departements as t 
						LEFT JOIN " . MAIN_DB_PREFIX . "c_regions ON t.fk_region = " . MAIN_DB_PREFIX . "c_regions.code_region
					WHERE t.active = 1 AND " . MAIN_DB_PREFIX . "c_regions.fk_pays = " . ($idpays > 0 ? $idpays : -1) . "
					ORDER BY t.nom
					";
