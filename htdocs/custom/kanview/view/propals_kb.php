				...
				switch ($newStatusID) {
					...
					case Propal::STATUS_SIGNED :
						if (compareVersions(DOL_VERSION, '14.0.0') == -1) { // < 14
							$res = @$object->cloture($user, $object::STATUS_SIGNED, '');	// dans PHP 7.2 $object->generateDocument() appelée par cette méthode provoque une exception TCPDI mais l'action se fait
						}
						else { // >= 14
						...
						if ($res > 0) {
							$response['status']	 = 'OK';
							$response['message'] = $langs->trans("PropalSuccessfullyClosedSigned");
							// setEventMessages($langs->trans("RecordUpdatedSuccessfully"), null);
						}
						break;
					...
					case Propal::STATUS_NOTSIGNED :
						if (compareVersions(DOL_VERSION, '14.0.0') == -1) { // < 14
							$res = @$object->cloture($user, Propal::STATUS_NOTSIGNED, '');	// dans PHP 7.2 $object->generateDocument() appelée par cette méthode provoque une exception TCPDI mais l'action se fait
						}
						else { // >= 14
							$res = $object->closeProposal($user, Propal::STATUS_NOTSIGNED, '');	
						}
						if ($res > 0) {
							$response['status']	 = 'OK';
							$response['message'] = $langs->trans("PropalSuccessfullyClosedNotSigned");
							// setEventMessages($langs->trans("RecordUpdatedSuccessfully"), null);
						}
						break;
...
if ($num >= 0) {
	...
	while ($i < $num) {
		...
		// filtre supplémentaire : Jours restants avant fin valididté
		// ce calcul n'a de sens que si la propale n'a pas encore été signée ni facturée
//		if (!empty($search_late)) {
//			$lateType = '';
//			if ($obj->fk_statut != Propal::STATUS_SIGNED && $obj->fk_statut != Propal::STATUS_BILLED) {
//				if (!empty($obj->fin_validite)) {
//					$retard = intval((intval($obj->fin_validite) - intval(dol_now('tzserver'))) / (60 * 60 * 24)); // retard en jours
//					if ($retard < 0) {
//						$lateType = 'late4'; // date fin valididté dépassé
//					}
//					elseif ($retard <= 7) {
//						$lateType = 'late3'; // moins que 7 jours avant fin valididité
//					}
//					elseif ($retard <= 15) {
//						$lateType = 'late2'; // moins que 15 jours avant fin valididité
//					}
//					elseif ($retard <= 30) {
//						$lateType = 'late1'; // moins que 30 jours avant fin valididité
//					}
//					else {
//						$lateType = 'other'; // plus que 30 jours avant fin valididité
//					}
//				}
//				else {
//					$lateType = 'other'; // date fin valididté non précisée
//				}
//			}
//			else {
//				$lateType = 'other'; // paropale signée et/ou facturée
//			}
//
...
if ($search_datep != '')
	$params	 .= '&amp;search_datep=' . urlencode($search_datep);
if ($search_fin_validite != '')
	$params	 .= '&amp;search_fin_validite=' . urlencode($search_fin_validite);
if ($search_date_cloture != '')
	$params	 .= '&amp;search_date_cloture=' . urlencode($search_date_cloture);
if ($search_fk_statut != '')
...
if ($search_date_livraison != '')
	$params	 .= '&amp;search_date_livraison=' . urlencode($search_date_livraison);
if ($search_societe_nom != '')
...
// ----------------- filtre -- Retard type
//
//echo '<tr id="tr-search_late" class="tr-external-filter"><td class="td-card-label">' . $langs->trans('KanviewPropalsFilter_Late') . '</td>';
//echo '<td>' . '' . '</td>';
...
//echo '<option value="late1" ' . ($search_late == 'late1' ? 'selected' : '') . '>' . $langs->trans('KanviewPropalsFilter_Late1') . '</option>';
//echo '<option value="late2" ' . ($search_late == 'late2' ? 'selected' : '') . '>' . $langs->trans('KanviewPropalsFilter_Late2') . '</option>';
//echo '<option value="late3" ' . ($search_late == 'late3' ? 'selected' : '') . '>' . $langs->trans('KanviewPropalsFilter_Late3') . '</option>';
//echo '<option value="late4" ' . ($search_late == 'late4' ? 'selected' : '') . '>' . $langs->trans('KanviewPropalsFilter_Late4') . '</option>';
//echo '<option value="other" ' . ($search_late == 'other' ? 'selected' : '') . '>' . $langs->trans('KanviewPropalsFilter_LateOther') . '</option>';
//echo '</select>';
//echo '</div>';
//echo '</td>';
//echo '</tr>';
